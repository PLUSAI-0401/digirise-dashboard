...
/**
 * 月別 新規獲得ユーザー数（プラン別内訳）+ ARPU を表示するカード
 */
export default function NewMembersByPlanCard({ newByPlan, activeByPlan, newTotal, year, month }) {
  ...
  return (
    <div className="chart-card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{
        ...
        ※ 広告経由 / Organic経由の判別は、Stripeにソース情報がないため未対応。必要な場合は決済時のmetadata付与で対応可能。
      </div>
    </div>
  );
}
